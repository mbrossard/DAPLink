...
error_t flash_manager_data(uint32_t addr, const uint8_t *data, uint32_t size)
{
    ...
    flash_manager_printf("flash_manager_data(addr=0x%x, size=%d (0x%x))\r\n", addr, size, size);
...
static error_t flush_current_block(uint32_t addr){
    ...
    if (!buf_empty) {
        ...
        flash_manager_printf("    intf->program_page(addr=0x%x, size=%d (0x%x)) ret=%i\r\n",
            current_write_block_addr, current_write_block_size, current_write_block_size, status);
        buf_empty = true;
    }
...
static error_t setup_next_sector(uint32_t addr)
{
    ...
    flash_manager_printf("        actual_write_size=%d (0x%x), sector_size=%d (0x%x), min_write=%d (0x%x)\r\n",
                         current_write_block_size, current_write_block_size, current_sector_size,
                         current_sector_size, min_prog_size, min_prog_size);
    return ERROR_SUCCESS;
}
#endif
